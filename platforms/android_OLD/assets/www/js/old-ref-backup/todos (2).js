...
//  var Todo = Parse.Object.extend("Gnomes", {
  ...
  var TodoView = Parse.View.extend({
	...
	checkTinCanComplete: function() {
		...
		console.log("actor: " + tincan.actor.name + " " + tincan.actor.mbox);
		  tincan.getStatements({
			  ...
			  callback: function (err, tinCanQuery) {
				  ...
				  if (tinCanQuery.statements.length > 0) {
					  console.log("Found " + tinCanQuery.statements.length + " LRS item(s) that match the checklist:"); 
					  ...
					  console.log("Item not found in LRS, so it is incomplete.");
						parseModel.save({"done": false});
					}
				  
				  else {
				  	console.log("Item in sync.");
				  }
			  }
			});	
  ...
  var ManageTodosView = Parse.View.extend({
    ...
    initialize: function() {
      ...
      state.on("change", this.filter, this);
		...
		tincan.actor = {name: Parse.User.current().attributes.username, mbox: Parse.User.current().attributes.email};
