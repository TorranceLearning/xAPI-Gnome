...
//			dispatcher.on("navprofileview:complete", this.completeAll, this);
			_.bindAll(this, 'scanQR', 'initCordovaPlugs', 'initBeacons', 'rangeBeacons', 'initBean', 'onNdef', 'decodePayload', 'refreshData', 'addOne', 'addAll', 'addSome', 'render', 'checkTinCanComplete', 'checkIfAllComplete', 'reloadPage');
...
//						console.log("Found " + tinCanQuery.statements.length + " LRS item(s) -------- >");

						//TODO fix this so it cycles through list first
						for (var i = 0; i < tinCanQuery.statements.length; i++) {
							var stmtActivity = tinCanQuery.statements[i].target.id;
//							console.log(stmtActivity);
								markComplete(stmtActivity);
						}
			...
			function markComplete(stmtActivity) {
				thisView.todos.each(function(item){
					var listActivity = item.attributes.objIDprefix + item.attributes.objID;
//								console.log(listActivity);
					if (stmtActivity === listActivity) {
						console.log("stmt match: " + stmtActivity);
//						console.log("marked complete: " + item.attributes.done);
